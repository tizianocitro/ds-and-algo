...
Your task is to find the maximum element from this array. Note that the array contains unique elements.
...
# solution one by finding the the peak
# Complexity:
...
# solution two by finding the minimum and returning its previous element
# Complexity:
# O(logn) time - where n is the number of elements in the input array
# O(1) space
class Solution:
    def findMin(self, nums) -> int:
        ...
        # at the end of the loop, left == right, so nums[left] points to the minimum
        # and we retunr nums[left - 1] that will point to the maximum
        return nums[left - 1]
